...
func (rpv rsPoolValue) refreshed(conn *DmConnection) (bool, error) {
	...
	tss, err := conn.Access.Dm_build_840(interface{}(rpv.m_TbIds).([]uint32))
	if err != nil {
		return false, err
	}
	rpv.m_lastChkTime = time.Now().Nanosecond()
...
func (st *DmStatement) prepare() error {
	...
	st.execInfo, err = st.dmConn.Access.Dm_build_765(st, Dm_build_1043)
	if err != nil {
		return err
	}
...
func (stmt *DmStatement) exec(args []driver.Value) (*DmResult, error) {
	...
	if stmt.isBatch && len(args) > 0 {
		...
		err = stmt.executeInner(args, Dm_build_1045)
	}
	if err != nil {
		return nil, err
	}
...
func (stmt *DmStatement) query(args []driver.Value) (*DmRows, error) {
	var err error
	stmt.inUse = true
	err = stmt.executeInner(args, Dm_build_1044)
	if err != nil {
		return nil, err
	}
...
func NewDmStmt(conn *DmConnection, sql string) (*DmStatement, error) {
	...
	if s == nil {
		...
		if conn.stmtPool != nil && len(conn.stmtPool) > 0 {
			...
			err := conn.Access.Dm_build_747(s)
			if err != nil {
				return nil, err
			}
		}
...
func (stmt *DmStatement) free() error {
	...
	err := stmt.dmConn.Access.Dm_build_752(int32(stmt.id))
	if err != nil {
		return err
	}
	delete(stmt.dmConn.stmtMap, stmt.id)
	...
	nextSwitch:
		if stmt.params[i].colType == CURSOR {
			if stmt.params[i].cursorStmt == nil {
				...
				err = stmt.params[i].cursorStmt.dmConn.Access.Dm_build_747(stmt.params[i].cursorStmt)
			}
...
func (stmt *DmStatement) executeInner(args []driver.Value, executeType int16) (err error) {
	...
	stmt.execInfo, err = stmt.dmConn.Access.Dm_build_797(stmt, bytes, false)
	if err != nil {
		return err
	}
		...
		for i, outParamData := range stmt.execInfo.outParamDatas {
			...
			if sc, ok := args[i].(sql.Scanner); ok {
				...
				case CURSOR:
					...
					if tmpExecInfo, err = stmt.dmConn.Access.Dm_build_807(stmt.params[i].cursorStmt, 1); err != nil {
						return err
					}
				...
				case *driver.Rows:
					if stmt.params[i].colType == CURSOR {
						var tmpExecInfo *execRetInfo
						tmpExecInfo, err = stmt.dmConn.Access.Dm_build_807(stmt.params[i].cursorStmt, 1)
						if err != nil {
							return err
						}
...
func (stmt *DmStatement) executeBatch(args []driver.Value) (err error) {
	...
	if stmt.execInfo.retSqlType == Dm_build_1058 || stmt.execInfo.retSqlType == Dm_build_1063 {
	...
	if stmt.paramCount > 0 && args != nil && len(args) > 0 {
		...
		if len(args) == 1 || stmt.dmConn.dmConnector.batchType == 2 ||
			(stmt.dmConn.dmConnector.batchNotOnCall && stmt.execInfo.retSqlType == Dm_build_1059) {
			...
			stmt.execInfo, err = stmt.dmConn.Access.Dm_build_786(stmt, bytes, stmt.preExec)
		}
	}
	return err
}
...
func (stmt *DmStatement) executeBatchByRow(args []driver.Value) (err error) {
	...
	for i := 0; i < count; i++ {
		tmpExecInfo, err := stmt.dmConn.Access.Dm_build_797(stmt, args[i].([]interface{}), stmt.preExec || i != 0)
		if err == nil {
